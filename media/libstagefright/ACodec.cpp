...
status_t ACodec::configureCodec(
    ...
    if (video) {
        ...
        if (encoder
            ...
            int32_t bitWidth = AUDIO_FORMAT_PCM_16_BIT;
#if defined(ENABLE_AV_ENHANCEMENTS) && defined(PCM_OFFLOAD_ENABLED_24)
            msg->findInt32("bits-per-sample", &bitWidth);
#endif
            err = setupRawAudioFormat(kPortIndexInput, sampleRate, numChannels, bitWidth);
        }
...
status_t ACodec::setupRawAudioFormat(
        OMX_U32 portIndex, int32_t sampleRate, int32_t numChannels, int32_t bitWidth) {
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = portIndex;
    ...
    pcmParams.nBitPerSample = bitWidth;
...
#if defined(ENABLE_AV_ENHANCEMENTS) && defined(PCM_OFFLOAD_ENABLED_24)
                    CHECK(params.nBitPerSample == 16u || params.nBitPerSample == 24u);
                    notify->setInt32("bits-per-sample", (params.nBitPerSample == 24u ?
