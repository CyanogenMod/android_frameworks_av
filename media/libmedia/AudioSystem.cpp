...
{
    ALOGV("AudioPolicyServiceClient::onOutputSessionEffectsUpdate(%d, %d, %d)", info->mStream, info->mSessionId, added);
    ...
    if (cb != NULL) {
        cb(AUDIO_OUTPUT_SESSION_EFFECTS_UPDATE, info->mStream, info->mSessionId, info->mFlags, info->mChannelMask, info->mUid, added);
    }
}
